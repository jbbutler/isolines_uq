...
# Specifically using the methodology which splits the sample in 2, using one half for isoline estimation
# and one half for bootstrapping.. Instead of the default where we transform the data via
# the ecdf + GPD blend, we just use the ecdf, specifically the rank transform with n+1 as denominator.
#
# Jimmy Butler
# February 2025
...
ns <- 2*c(1000, 5000, 10000, 50000) # sample sizes to test, 2 times because sample will be split in half..
...
parallelizedCode <- function(ind) {
    ...
    regions <- computeExtremeRegionSplit(dat=transformed_dat,
                                 alphas=alphas,
                                 p=p,
                                 B=B,
                                 gamma=gamma,
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        n <- ns[k]
        p <- pn(n)*2 # times 2 because we want to compare with using two different samples of size n
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, 'extremal_coverage_twowaysplit_rank_marg.csv')
