...
parallelizedCode <- function(ind) {
    ...
    # sample dataset
    input_dat <- sampling_func(n)
    regions <- computeExtremeRegion(dat=input_dat,
                                 alphas=alphas,
                                 p=p,
                                 B=B,
                                 gamma=gamma,
    ...
    is_covereds <- evaluateCoverage(regions, isoline, extreme=TRUE)
    # add into the list
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, 'extremal_coverage_asymmetric.csv')
