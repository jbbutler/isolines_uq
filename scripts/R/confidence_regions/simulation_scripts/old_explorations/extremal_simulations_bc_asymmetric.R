...
gamma <- 2/3 # controls level to project from for extremal survival function
gamma1 <- 1/3 # controls level to project from for bias upper bounding
gamma2 <- 1/2 # controls level to project to for bias upper bounding

B <- 2000 # number of bootstrap iterations for each confidence tube (computing tube parameters)
B_bias <- 500 # number of bootstrap iterations for estimating bias upper bound
...
parallelizedCode <- function(ind) {
    ...
    # sample dataset
    dat <- sampling_func(n)
    # compute levels to project from and to for bias upper bounding
    q1 <- n^(-gamma1)
    q2 <- n^(-gamma2)
    regions <- computeExtremeRegionBC(dat=dat, alphas=alphas, p=p, B=B, B_bias=B_bias, 
                                   gamma=gamma, q1=q1, q2=q2, ncoords=50, 
        ...
        if (!is.null(args$save_df_path)) {
            ...
            coverage_df <- do.call(rbind, lapply(samp_regions, function(res) {
                data.frame(
                    ...
                    c_plus_estimate = unlist(res$c_plus_estimates[names(res$is_covereds)]),
                    c_minus_estimate = unlist(res$c_minus_estimates[names(res$is_covereds)])
                )
            }))

            # determine if we need to write headers (only if file doesn't exist)
            save_fname <- paste0(args$save_df_path, 'extremal_coverage_bc_asymmetric.csv')
