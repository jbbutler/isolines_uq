# Script to evaluate the coverage properties of the confidence tube method using the extreme
# survival function, where instead of using the empirical marginal distribution functions for the
# Pareto transform, we use kernel density estimates that will likely be lower variance.
# This is similar to the marginal transformation procedure in Cooley.
#
# Jimmy Butler
# February 2025
...
gamma <- 1/2 #rate to look out into tail
xi <- 1 #extreme index for regular variation projection 
pn_funcs <- list()
pn_funcs[['p5_div_n']] = function(n) { 5/n }
...
parallelizedCode <- function(ind) {
    ...
    # sample dataset
    dat <- sampling_func(n)
    # transform dataset to have (approximate) pareto 1 marginals
    transform <- function(pts, dat) {
        transformed_pts <- 1/(1-est_cdf(pts, dat, gamma, method='kde')) - 1
    ...
    regions <- computeExtremeRegion(dat=transformed_dat,
                                     alphas=alphas,
                                     p=pn(n),
                                     B=B,
                                     gamma=gamma,
    ...
    transformed_isoline <- data.frame(X1=transform(isoline[,1], dat[,1]),
                                      X2=transform(isoline[,2], dat[,2]))
...
for (i in 1:length(dists)) {
    ...
    for (j in 1:length(pn_funcs)) {
        pn <- pn_funcs[[j]]
        pn_name <- names(pn_funcs)
        # only create p directory if save_full_path is provided
        if (!is.null(save_full_path)) {
            ppath <- paste0(distpath, pn_name[j], '/')
            if (!dir.exists(ppath)) dir.create(ppath, recursive = TRUE)
        }
        
        print(paste0('Starting new p_func: ', pn_name))
        
        for (k in 1:length(ns)) {
            # create the true p-isoline for this distribution
            n <- ns[k]
            isoline <- drawIsoline(dist=dist, numCoords=200, gridUbs=ubs, gridLbs=lbs, prob=pn(n))
            ...
            if (!is.null(args$save_df_path)) {
                ...
                save_fname <- paste0(args$save_df_path, 'extreme_coverage_kde.csv')
