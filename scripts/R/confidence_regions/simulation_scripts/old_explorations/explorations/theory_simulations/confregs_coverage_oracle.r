...
path <- '/pscratch/sd/j/jbbutler/oracle_analysis/bivt/'
...
for (i in 1:length(ps)) {
    isolines[[as.character(ps[i])]] <- drawBivtIsoline(numCoords=200, gridUbs=c(10,10), gridLbs=c(-2,-2), prob=ps[i])
}
...
for (i in 1:length(ns)) {
    ...
    for (j in 1:length(ps)) {
        ...
        for (k in 1:length(alphas)) {
            ...
            saveRDS(results, file=paste0('/pscratch/sd/j/jbbutler/oracle_analysis/bivt/covrates/p', p, '_alpha', alpha, '_n', n, '.RData'))
