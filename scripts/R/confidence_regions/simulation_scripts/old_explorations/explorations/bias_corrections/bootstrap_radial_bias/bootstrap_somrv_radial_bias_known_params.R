...
C <- 5
xi <- 1/4 # assume we know the extreme value index
rho <- -1/2 # assume we know the second-order MRV parameter
gamma1 <- 1/2
gamma2 <- 2/3
...
estimateSomrvBootstrapBias <- function(dat, p, q1, q2, B, n_coords, gamma1, gamma2, xi, rho, lbs) {
    ...
    # compute expectation of projection estimator from q1 to q2 IN THE BOOTSTRAP WORLD
    for (k in 1:B) {
        idx <- sample.int(n_dat, n_dat, replace=TRUE)
        boot_dat <- dat[idx, , drop=FALSE]
        
        # project from q1 to q2 using the bootstrap sample
        boot_isoline_q2 <- drawExtremeIsoline(dat=boot_dat, p=q2, n_coords=n_coords, grid_lbs=lbs, gamma=gamma1, xi=xi)
    ...
    # compute the bias true bias ratio of q1 to p bias to q1 to q2 bias (assumes oracle knowledge of xi and rho)
    ...
    for (k in 1:length(ns)) {
        ...
        parallelizedCode <- function(ind, true_bias, q1, q2) {
            ...
            somrv_res <- estimateSomrvBootstrapBias(dat=dat, p=p, q1=q1, q2=q2, B=B_boot, n_coords=n_coords, 
                                                    gamma1=gamma1, gamma2=gamma2, xi=xi, rho=rho, lbs=lbs)
        ...
        save_fname <- paste0(args$save_df_path, 'somrv_simulation2.csv')
