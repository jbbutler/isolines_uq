...
# doing no transforms, no sample splitting, and no bias correction. Mostly serving as a basis of comparison
# with bias correction strategies to see how much better things are.
#
# Jimmy Butler
# March 2026
...
computeExtremeRegion <- function(dat, alphas, p, B, gamma, xi, n_coords, lbs=c(0,0), progress_bar=FALSE) {

    # construct the bias-corrected isoline to take the supremum over
    construction_dat <- dat
    ext_region_pts <- drawExtremeIsoline(dat=construction_dat, p=p, n_coords=n_coords, grid_lbs=lbs, gamma=gamma, xi=xi)
    ...
    for (k in 1:length(ns)) {
        ...
        parallelizedCode <- function(ind) {
            ...
            regions <- computeExtremeRegion(dat=dat, alphas=alphas, p=p, B=B, 
                                                          gamma=gamma, xi=xi, n_coords=n_coords)
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, 'extremal_coverage_nobc_notransform.csv')
