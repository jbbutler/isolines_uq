# Script to compute coverage rates of confidence tube procedure, correcting the bias in the isoline estimates
# and taking the suprema/infima over this bias corrected curve.
# Here, we assume we sample from a bivarate t, and that we know the index of regular variation.
#
# Jimmy Butler
# March 2026
...
B <- 2000
n_sims <- 500
n_monte_carlo <- 2000 # number of Monte Carlo samples to compute expected radii at each angle theta
n_coords <- 100
...
# a function to compute an extreme region given true radial biases
computeExtremeRegionRadialBC <- function(dat, alphas, p, B, gamma, xi, b_thetas, n_coords, lbs=c(0,0), progress_bar=FALSE) {

    # construct the bias-corrected isoline to take the supremum over
    construction_dat <- dat
    isoline_estimate <- drawExtremeIsoline(dat=construction_dat, p=p, n_coords=n_coords, grid_lbs=lbs, gamma=gamma, xi=xi)
    theta <- atan2(isoline_estimate[,2], isoline_estimate[,1])
    r_hats <- sqrt(isoline_estimate[,1]**2 + isoline_estimate[,2]**2)
    r_tildes <- r_hats - b_thetas
    
    ext_region_pts <- data.frame(X1=r_tildes*cos(theta), X2=r_tildes*sin(theta))
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        ### Computing True Radial Biases at Each Angle via Monte Carlo Simulation ###
        ...
        for (i in 1:n_monte_carlo) {
            dat <- sampling_func(n)
            isoline_ext_estimate <- drawExtremeIsoline(dat=dat, p=p, n_coords=n_coords, grid_lbs=lbs, gamma=gamma, xi=xi)
            r_hat_thetas <- sqrt(isoline_ext_estimate[,1]**2 + isoline_ext_estimate[,2]**2)
            r_hat_theta_mat[i,] <- r_hat_thetas
        }
        # radial biases at each angle, will save these for computing confidence tubes
        ...
        parallelizedCode <- function(ind) {
            ...
            # use the new radial bias corrected function
            regions <- computeExtremeRegionRadialBC(dat=dat, alphas=alphas, p=p, B=B, 
                                                          gamma=gamma, xi=xi, b_thetas=b_thetas, n_coords=n_coords)
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, 'extremal_coverage_true_radial_bc_no_transform_gamma_0.5.csv')
