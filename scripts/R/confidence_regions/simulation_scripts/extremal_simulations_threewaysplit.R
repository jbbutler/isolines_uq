...
# Specifically using the methodology which splits the sample in 3, using one third for marginal
# estimation, one third for isoline estimation, and one third for bootstrapping
#
# Jimmy Butler
# February 2025
...
parallelizedCode <- function(ind) {
    ...
    regions <- computeExtremeRegionThreewaySplit(dat=dat,
                                 alphas=alphas,
                                 p=p,
                                 B=B,
                                 gamma=gamma,
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        n <- ns[k]
        p <- pn(n)*3 # times 3 because we want to compare with using three different samples of size n
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, 'extremal_coverage_threeway_split.csv')
