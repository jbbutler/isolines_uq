...
#
# LOG-SCALE VERSION: the Mammen-Polonik band is calibrated on the LOG of the survival estimator
# (as in the extreme-value scripts), so the tube is multiplicative and the outer wall
# p*exp(c_minus_log) is always > 0 -- the additive blow-up cannot occur.
#
# Also computes tube SIZE (absolute + relative radial width) for each simulated tube.
#
# Jimmy Butler
# June 2026
...
parser <- ArgumentParser(description = "Args for empirical survival function simulations (Log-scale, Asymmetric).")
...
computeEmpiricalRegion <- function(dat, alphas, p, B, lbs=c(0,0), ncoords=n_coords, verbose=FALSE) {
    ...
    # Floor for the log transform: the empirical survival can be exactly 0 in a
    # resample (no resampled points beyond the isoline along a ray), and log(0) = -Inf.
    # Half a count (0.5 / n) is a standard continuity correction on the survival scale.
    ...
    for (k in 1:B) {
        ...
        # LOG-SCALE: store the log of the (floored) bootstrap survival probs
        boot_probs_mat[k, ] <- log(pmax(emp_probs, prob_floor))
        
        if (verbose) utils::setTxtProgressBar(pb, k)
    }
    if (verbose) close(pb)
    
    # Bootstrap Centering in LOG space (center on the bootstrap mean, per angle)
    mean_log_boot_probs <- colMeans(boot_probs_mat)
    boot_log_devs <- sweep(boot_probs_mat, MARGIN = 2, STATS = mean_log_boot_probs, FUN = "-")
    
    # Extract global max and min LOG-deviations across the curve for each draw
    ...
    #    Calibrate quantiles in log space, then map back to ADDITIVE survival-deviation
    #    units (survival wall = p + c) so the size / coverage / dataframe code is unchanged.
    #    Outer wall p*exp(c_minus_log) is always > 0, so the tube never blows up.
    for (i in 1:length(alphas)) {
        alpha <- alphas[i]
        c_plus_log  <- as.numeric(quantile(boot_draws_Wplus,  probs=1-(alpha/2), type=1))
        c_minus_log <- as.numeric(quantile(boot_draws_Wminus, probs=alpha/2,     type=1))

        c_plus_estimates[[as.character(alpha)]]  <- p * exp(c_plus_log)  - p
        c_minus_estimates[[as.character(alpha)]] <- p * exp(c_minus_log) - p
    }
    ...
    for (i in 1:length(alphas)) {
        ...
        # Unboundedness check retained for interface parity; on the log scale
        # cminus = p*exp(c_minus_log) - p > -p always, so this never triggers.
        if (cminus <= -p) {
        ...
        for (label in names(p_funcs[[regime]])) {
            ...
            for (k in 1:length(ns)) {
                ...
                if (!is.null(args$save_df_path)) {
                  ...
                  save_fname <- paste0(args$save_df_path, 'empirical_simulations_logscale.csv')
