...
# with t_4 and t_10 margins (heavy-tailed; for comparability with the AD
# t4/t10 results). Per-coordinate (Cooley) transition, full-space truth.
...
rho_corr <- 0.7
beta_cooley <- 200
marg_models <- c("gpd", "gpd")         # (t_4, t_10)

dists <- c('gausscopula_t4t10margs')
lbs <- c(0, 0)
ubs <- c(200, 200)
...
# sampler: Gaussian copula (rho = 0.7) -> t_4 and t_10 margins
sample_gausscopula_t4t10 <- function(n) {
    Sigma <- matrix(c(1, rho_corr, rho_corr, 1), 2, 2)
    X <- mvtnorm::rmvnorm(n, mean = c(0, 0), sigma = Sigma)
    U <- pnorm(X)
    data.frame(X1 = qt(U[,1], df = 4), X2 = qt(U[,2], df = 10))
}
...
for (i in 1:length(dists)) {
    dist <- dists[i]
    sampling_func <- sample_gausscopula_t4t10
    ...
    for (k in 1:length(ns)) {
        ...
        isoline <- data.frame(X1 = qt(U_iso[,1], df = 4), X2 = qt(U_iso[,2], df = 10))
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '1.1.1.2_bivgausscopula_t4t10margs_gamma1_0.5_gamma2_0.67_bootmargs_cooleysmoothpercoord.csv')
