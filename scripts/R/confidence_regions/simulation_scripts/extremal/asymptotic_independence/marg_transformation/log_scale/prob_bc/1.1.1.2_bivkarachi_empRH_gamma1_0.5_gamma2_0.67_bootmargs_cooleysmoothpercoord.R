# 1.1.1.2 AI tubes: KARACHI beta-KDE (temp/relHum, asymptotic INDEPENDENCE).
# Per-coordinate (Cooley) transition, full-space truth. SINGLE TUBE --
#
# marg_models = c("gpd", "empirical"): temp keeps the empirical-body + GPD-tail
# blend; relative humidity is transformed by the tie-aware EMPIRICAL CDF, which
# removes the xi_2 ~ -1 endpoint-collapse degeneracy (the RH margin saturates
# at 100 and its GPD-MLE is nonregular). This severs the margin-degeneracy
# channel (b) so that any residual coverage failure is attributable to the
# small-eta transition geometry (channel c) rather than the transform.
#
...
parser <- ArgumentParser(description = "Dual AI tubes on the Karachi beta-KDE: per-coordinate Cooley transition (full) + pure-HRV radial (interior); GPD temp margin + empirical RH margin.")
...
# --- method parameters ---
beta_cooley <- 200
marg_models <- c("gpd", "empirical")     # (temp, relHum) -- RH via empirical CDF     # (temp, relHum); see header
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '1.1.1.2_bivkarachi_empRH_gamma1_0.5_gamma2_0.67_bootmargs_cooleysmoothpercoord.csv')
