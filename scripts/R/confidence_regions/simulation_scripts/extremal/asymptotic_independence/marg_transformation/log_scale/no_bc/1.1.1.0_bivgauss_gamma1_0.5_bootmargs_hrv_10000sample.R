# 1.1.1.0 ANGLE-RESOLVED MISCOVERAGE diagnostic (bivgauss, n = 10000 ONLY):
# pure-HRV radial tube on the INTERIOR cone, raw walls only -- per-sim,
...
# coverage harness, verbatim), evaluate the tube's survival estimator at every
# INTERIOR truth point (true pseudo-angle in [w_int, 1 - w_int]), and record
...
#
# Truth is a DENSE 200-point isoline (diagnostic resolution), independent of
# the 50-point tube theta grid.
#
...
dists <- c('bivgauss')
lbs <- c(0, 0)
ubs <- c(6, 6)                     # standard-normal margins: truth grid
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        isoline <- drawIsoline(dist = dist, numCoords = n_truth_coords, gridUbs = ubs, gridLbs = lbs, prob = p)
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '1.1.1.0_bivgauss_gamma1_0.5_bootmargs_hrv_misscoverage_n10000.csv')
