...
# the SINGLE-TUBE Gaussian-copula AI simulations (1.1.1.2 bivgausscopula
# unifbeta52margs bootmargs_cooleysmoothpercoord): per-coordinate (Cooley)
# full-space tube only -- no interior / hrv rows.
#
# Loads each saved 'regions' object, reconstructs the percoord survival
# estimator from the stored fits / eta_hat / thetas -- the saved survFunc
# closure does not survive readRDS in a fresh session -- swaps the tube's RAW
# walls into the evaluation slots, and re-evaluates coverage against the same
# true isoline (Uniform(0,1) / Beta(5,2) margins, Gaussian copula rho = 0.7).
#     percoord_full   (raw percoord tube vs full truth)
#
# All machinery is inlined verbatim from the simulation script (self-contained;
# per-margin gpd/empirical dispatch preserved). marg_models = c("gpd", "gpd").
#
...
# NOTE: point save_full_path at wherever the 1.1.1.2 unifbeta52margs tubes were saved.
save_full_path <- '/pscratch/sd/j/jbbutler/isolines/simulations/extremal/1.1.1.2_bivgausscopula_unifbeta52margs_gamma1_0.5_gamma2_0.67_bootmargs_cooleysmoothpercoord/'
...
# reconstruct a full marginal fit (add x_sorted, n, empirical grid) from a
# trimmed stored fit + data, dispatching on the stored model.
rebuild_fit <- function(fit_trim, x) {
    n <- length(x); xs <- sort(x)
    fit_trim$n <- n; fit_trim$x_sorted <- xs
    if (is.null(fit_trim$model)) fit_trim$model <- "gpd"
    if (identical(fit_trim$model, "empirical")) {
        pos_all <- (seq_len(n) - 0.5) / n
        ux <- unique(xs)
        fit_trim$ux  <- ux
...
# ======================================================================
results_list <- list()
print("Evaluating RAW single tube: gausscopula unifbeta52margs percoord...")
...
for (dist in dists) {
  for (n in ns) {
    ...
    # TRUE p-isoline: same chain as the simulation script
    isoline <- drawIsoline(dist = 'bivgauss', numCoords = n_coords, gridUbs = ubs, gridLbs = lbs, prob = p)
    ...
    for (i in seq_along(sim_files)) {
      ...
      dat <- regions$dat
      fit1 <- rebuild_fit(regions$fit1, dat[,1]); fit2 <- rebuild_fit(regions$fit2, dat[,2])
      qn <- regions$fit1$q1
      Z  <- standardize(dat[,1], dat[,2], fit1, fit2)
      eta_hat <- regions$eta_hat

      # reconstruct the percoord survFunc
      sf_pc <- function(pts) { pts <- as.matrix(pts)
          zq <- standardize(pts[,1], pts[,2], fit1, fit2)
          std_blended_surv_pc(zq, Z, qn, regions$thetas, eta_hat, beta_cooley) }

      obj <- list(survFunc = sf_pc, p = p,
      ...
      results_list[[length(results_list) + 1]] <- data.frame(
          dist = dist, p = p, n = n, method = 'percoord_full', alpha = ak,
...
if (length(results_list) > 0) {
    ...
    save_fname <- paste0(save_df_path, '1.1.1.0_bivgausscopula_unifbeta52margs_gamma1_0.5_bootmargs_cooleysmoothpercoord.csv')
