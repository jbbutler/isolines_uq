# 0.1.1.2 AI tubes: bivariate-GAUSSIAN COPULA (rho = 0.7, asymptotic INDEPENDENCE)
# with Uniform(0,1) and Beta(5,2) margins. Per-coordinate (Cooley) transition,
#
# Margins: Uniform is exactly GPD(xi = -1) above any threshold; to avoid the
# unbounded-MLE degeneracy at xi = -1 it is transformed by the tie-aware
# EMPIRICAL CDF. Beta(5,2) (xi = -1/6 interior) uses the gpd blend.
# marg_models = c("empirical", "gpd").
#
...
rho_corr <- 0.7
beta_cooley <- 200
marg_models <- c("gpd", "gpd")   # (Uniform, Beta(5,2))

dists <- c('gausscopula_unifbeta52margs')
lbs <- c(0, 0)
ubs <- c(200, 200)
...
# sampler: Gaussian copula (rho = 0.7) -> Uniform(0,1) and Beta(5,2) margins
sample_gausscopula_unifbeta52 <- function(n) {
    Sigma <- matrix(c(1, rho_corr, rho_corr, 1), 2, 2)
    X <- mvtnorm::rmvnorm(n, mean = c(0, 0), sigma = Sigma)
    U <- pnorm(X)
    data.frame(X1 = U[,1],                              # Uniform(0,1) = identity on U
               X2 = qbeta(U[,2], shape1 = 5, shape2 = 2))
}
...
# ---------- marginal machinery (per-margin gpd/empirical dispatch) ----------
...
computeRegion_Percoord <- function(dat, alphas, p, B, gamma1 = 1/2, gamma2 = 2/3,
    ...
    ## ---- PERCOORD tube, full space (Cooley transition) ----
    ...
    ## ---- copula-channel (beta) pad, measured at q2 on the percoord q2-curve ----
    zq2_pc <- pc_pts(q1 / q2)
    emp_q2 <- colMeans(outer(Z[,1], zq2_pc[,1], ">") & outer(Z[,2], zq2_pc[,2], ">"))
    bft <- (emp_q2 - q2) / (q2 * log(q1 / q2)) * log(q1 / p)
    bf_pos <- max(0, max(bft)); bf_neg <- max(0, -min(bft))
    ...
    for (alpha in alphas) {
        ...
        if (s_lo <= 0) {
            ...
            wi <- wall_pc(s_hi); wo <- wall_pc(s_lo)
            sizes[[ak]] <- cbind(data.frame(is_unbounded = FALSE),
                computeTubeSize(rad(wi$orig), rad(wo$orig), rad(wall_pc(p)$orig)))
        }
    }

    survFunc <- function(pts) { pts <- as.matrix(pts)
    ...
    list(dat = dat, p = p, gamma1 = gamma1, gamma2 = gamma2,
         ...
         eta_hat = eta_hat, beta_cooley = beta_cooley, marg_models = marg_models,
         b_marg = b_marg, B_marg1 = B_marg1, B_marg2 = B_marg2,
         fallback_frac = n_fallback / (2 * B), clamp_frac = n_clamp / B,
         thetas = thetas_full, x_iso = x_iso_pc, projection = "percoord",
         c_plus_estimates = cps, c_minus_estimates = cms,
         raw_c_plus_estimates = rcps, raw_c_minus_estimates = rcms,
         size_estimates = sizes, survFunc = survFunc,
         beta_frac_pos = bf_pos, beta_frac_neg = bf_neg, transformed = FALSE)
}
### END CORE ###########################################################

# ======================================================================
# Main simulation loop (single tube: percoord Cooley, full-space truth)
# ======================================================================
for (i in 1:length(dists)) {
    dist <- dists[i]
    sampling_func <- sample_gausscopula_unifbeta52
    ...
    for (k in 1:length(ns)) {
        ...
        # TRUE p-isoline (full)
        isoline <- drawIsoline(dist = 'bivgauss', numCoords = n_coords, gridUbs = c(200,200), gridLbs = c(0,0), prob = p)
        # bivgauss truth is on the Gaussian-copula scale; map to the marginal scale
        U_iso   <- pnorm(as.matrix(isoline))
        isoline <- data.frame(X1 = U_iso[,1], X2 = qbeta(U_iso[,2], shape1 = 5, shape2 = 2))
        ...
        parallelizedCode <- function(ind) {
            dat <- sampling_func(n)
            regions <- computeRegion_Percoord(dat = dat, alphas = alphas, p = p, B = B,
                                              gamma1 = gamma1, gamma2 = gamma2,
                                              ncoords = n_coords, lbs = lbs,
                                              beta_cooley = beta_cooley, marg_models = marg_models)

            obj <- list(survFunc = regions$survFunc, p = p,
                        c_plus_estimates = regions$c_plus_estimates,
                        c_minus_estimates = regions$c_minus_estimates,
                        transformed = FALSE)
            ...
            ak <- names(covs)
            szs <- do.call(rbind, regions$size_estimates[ak])
            data.frame(dist = dist, p = p, n = n, method = 'percoord_full', alpha = ak,
                is_covered = unlist(covs, use.names = FALSE),
                c_plus_adj  = unlist(regions$c_plus_estimates[ak]),
                c_minus_adj = unlist(regions$c_minus_estimates[ak]),
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '1.1.1.2_bivgausscopula_unifbeta52margs_gamma1_0.5_gamma2_0.67_bootmargs_cooleysmoothpercoord.csv')
