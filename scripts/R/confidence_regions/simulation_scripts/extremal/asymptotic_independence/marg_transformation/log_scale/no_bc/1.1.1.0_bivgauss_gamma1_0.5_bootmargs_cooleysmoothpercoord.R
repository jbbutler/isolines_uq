...
# the DUAL-TUBE bivariate Gaussian simulations (1.1.1.2 bivgauss
# bootmargs_cooleysmoothpercoord): per-coordinate full-space tube + pure-HRV
# interior tube.
#
# Loads each saved 'regions' object, reconstructs BOTH survival estimators
# (percoord full + hrv-const interior) from the stored fits/eta -- the saved
# survFunc closures do not survive readRDS in a fresh session -- swaps each
# tube's RAW walls into the evaluation slots, and re-evaluates the same three
# coverage rows against the same interior-truth cut:
#     percoord_full      (raw percoord tube vs full truth)
#     percoord_interior  (same raw tube vs interior truth subset)
#     hrv_interior       (raw hrv-const tube vs interior truth subset)
#
...
# NOTE: point save_full_path at wherever the 1.1.1.2 bivgauss dual tubes were saved.
save_full_path <- '/pscratch/sd/j/jbbutler/isolines/simulations/extremal/1.1.1.2_bivgauss_gamma1_0.5_gamma2_0.67_bootmargs_cooleysmoothpercoord/'
...
rebuild_fit <- function(fit_trim, x) {
    fit_trim$n <- length(x); fit_trim$x_sorted <- sort(x)
...
# ======================================================================
results_list <- list()
print("Evaluating RAW dual tubes: bivgauss percoord + hrv interior...")
...
for (dist in dists) {
  for (n in ns) {
    ...
    # TRUE p-isoline (full) + interior subset via TRUE pseudo-angle (exact Gaussian margins)
    ...
    for (i in seq_along(sim_files)) {
      ...
      # raw-wall evaluation for one tube
...
if (length(results_list) > 0) {
    ...
    save_fname <- paste0(save_df_path, '1.1.1.0_bivgauss_gamma1_0.5_bootmargs_cooleysmoothpercoord.csv')
