...
# Temperature-humidity is asymptotically independent; the RH margin is bounded
# and SATURATING (mass at/near 100), which makes a free GPD fit degenerate
# (xi <= -1, endpoint collapse) and corrupts the standardized minima feeding
# eta_hat. Hence the per-margin model choice below.
#
# MARGINAL MODELS (marg_models): per-margin transform choice.
#   temp  : "gpd"       -- healthy fit (xi ~ -0.18), genuine extrapolation risk
#   relHum: "empirical" -- interpolated, tie-aware empirical CDF (Hazen
#           positions, averaged over ties), ONE coherent forward/inverse pair.
#           Every marginal level the tube queries is within-sample here, so no
#           tail model is needed -- and the degenerate GPD is thereby removed
#           from the pipeline. Set c("gpd","gpd") to reproduce the pathology.
#
# Per simulated dataset, TWO tubes from the SAME data/fits/eta/bootstrap:
#   (1) PERCOORD (full space): Cooley-exact per-coordinate transition
#       e1(w) = eta + (1-eta) w^beta, e2(w) = eta + (1-eta)(1-w)^beta,
#       applied to the isotonized q1 anchor curve (fold-free by construction).
#   (2) HRV-INTERIOR: pure radial exponent eta, no transition, interior cone.
# Coverage rows (method column): percoord_full, percoord_interior (same tube,
# interior truth), hrv_interior -- the three-way decomposition of arm vs
# transition vs interior behavior.
#
...
#
# CLAMP DIAGNOSTIC REDEFINED: clamp_frac is now the average FRACTION of
# standardized points at the ceiling 2n per draw (the old any-point indicator
# saturates trivially under an empirical margin, whose sample max maps to 2n
# by construction). Expected magnitude ~ (# ties at the max)/n.
#
...
computeRegion_Dual_HRV <- function(dat, alphas, p, B, gamma1 = 1/2, gamma2 = 2/3,
                                   ...
                                   w_int = 0.02, marg_models = c("gpd", "gpd")) {
    n_dat <- nrow(dat)
        ...
        for (alpha in alphas) {
            ...
            if (s_lo <= 0) {
                ...
                sizes[[ak]] <- cbind(data.frame(is_unbounded = FALSE),
                    computeTubeSize(rad(wall_pts(s_hi)), rad(wall_pts(s_lo)), rad(wall_pts(p))))
            }
        }
    ...
    wall_pc  <- function(s) backmap(pc_pts(q1 / s), fit1, fit2)
    wall_int <- function(s) backmap(cbind(rq1_int * (q1/s)^eta_hat * cos(thetas_int),
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '1.1.1.2_bivkarachi_gamma1_0.5_gamma2_0.67_bootmargs_cooleysmoothpercoord.csv')
