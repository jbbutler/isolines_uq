...
# LIGHT UNEQUAL TAILS: Gaussian margins with variances 5 and 10 (both xi = 0,
# short/light tails), t_4-copula dependence. Stress-tests the marginal-transform
# AD procedure when margins are LIGHTER than heavy-tailed and unequal -- the GPD
# shape should estimate near 0 (Gumbel domain), the delicate small-|xi| regime.
#
...
parser <- ArgumentParser(description = "Marginal-transform AD tube sims: light unequal Gaussian tails (var 5, 10), t_4 copula, GPD margins, Pareto standardization.")
...
# sampler: t_4-copula with N(0,5) and N(0,10) margins. Copula from a bivariate t_4,
# mapped to uniforms via the t_4 CDF, then through the per-margin Gaussian quantiles.
sample_t4copula_gauss <- function(n) {
    Sigma <- matrix(c(1, 0.7, 0.7, 1), 2, 2)
    Tm <- mvtnorm::rmvt(n, sigma = Sigma, df = copula_df)
    U  <- pt(Tm, df = copula_df)
    X1 <- qnorm(U[,1], mean = 0, sd = sd1)
    X2 <- qnorm(U[,2], mean = 0, sd = sd2)
...
for (i in 1:length(dists)) {
    dist <- dists[i]
    sampling_func <- sample_t4copula_gauss
    ...
    for (k in 1:length(ns)) {
        ...
        # Gaussian quantiles. Same t_4 copula as the data; margins N(0,5), N(0,10).
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '0.1.1.2_bivt4_gauss5gauss10margs_gamma1_0.5_gamma2_0.67_bootmargs.csv')
