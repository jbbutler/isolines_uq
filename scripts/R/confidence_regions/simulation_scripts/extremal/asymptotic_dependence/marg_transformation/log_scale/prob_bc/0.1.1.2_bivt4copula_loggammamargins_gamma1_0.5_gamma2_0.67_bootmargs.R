...
#
# DISTRIBUTION: t_4-copula with LOG-GAMMA margins (marginal xi = 1/4, marginal rho = 0).
#
# THE DECISIVE TEST for the marginal-transform procedure: at bivariate t_4 the raw
# (uncorrected) tubes stabilized at nominal because the residual biases were small.
# Here the MARGINAL channel is the slow one -- the GPD misfit to a rho = 0 tail
# decays only logarithmically -- so the predictions are:
#   * B_marg1/B_marg2 light up relative to the bivt run;
#   * RAW coverage degrades with n (slow log-rate; may need the larger n's to see);
#   * CORRECTED coverage holds -- the marginal pad finally load-bearing
#     rather than insurance;
#   * xi_gpd1/2 persistently above 0.25 (penultimate absorption of the log factor).
#
# Pipeline per simulation (identical to 0.1.1.2; only sampler + truth differ):
...
parser <- ArgumentParser(description = "Marginal-transform AD tube sims: t4-copula + log-gamma margins, GPD margins, Pareto standardization.")
...
# --- log-gamma / copula parameters (data-generating) ---
# X = exp(G), G ~ Gamma(shape=a, rate=b):  Fbar(x) ~ C x^{-b} (log x)^{a-1}
#   marginal tail index 1/xi = b  => b = 4 gives xi = 1/4
#   a != 1 gives a genuine slowly-varying log factor => marginal rho = 0
lg_shape  <- 2      # a
lg_rate   <- 4      # b = 1/xi
...
# sampler: t_4-copula + log-gamma margins (margins positive since X = exp(G) > 0)
...
# marginal quantile for the log-gamma (for the TRUE isoline)
...
# (The procedure never sees the true marginal form -- the GPD has to cope
#  with the rho = 0 tail on its own. That is the test.)
# ======================================================================
...
computeRegion_MargTransform_AD <- function(dat, alphas, p, B, gamma1 = 1/2, gamma2 = 2/3,
    ...
    for (i in 1:length(alphas)) {
        ...
        if (s_lo <= 0) {   # inert on the log scale; kept for interface parity
            ...
            # empirical minima at the axes don't pollute the radii
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        # (the truth is on the ORIGINAL scale; survFunc composes the transform internally)
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '0.1.1.2_bivt4copula_loggammamargins_gamma1_0.5_gamma2_0.67_bootmargs.csv')
