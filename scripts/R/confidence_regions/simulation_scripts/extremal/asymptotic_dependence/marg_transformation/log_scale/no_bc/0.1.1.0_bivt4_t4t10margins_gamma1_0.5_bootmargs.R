# Script to loop through saved MARGINAL-TRANSFORM (AD) t4-copula tubes with
# ASYMMETRIC t_4 / t_10 margins and evaluate un-bias-corrected coverage.
#
# Loads the saved tube objects from the 0.1.1.2 bivt4_t4t10margins bootmargs
# simulations, swaps the RAW (variance-only) walls into the evaluation slots,
# and re-evaluates coverage of the UNCORRECTED tubes against the true
# (original-scale) asymmetric-margin isoline. Also records raw tube sizes.
#
# PREDICTION ON RECORD: both channels are fast here (t_4 AND t_10 margins have
# quantile-scale rho = -1/2; the t_4-copula channel likewise), so raw coverage
# should stabilize at nominal as in the symmetric bivt4 run -- the asymmetry
# itself should cost nothing, since standardization erases it. If raw covers,
# that completes the demonstration that the procedure handles unequal tail
# heaviness for free; if it does not, the asymmetry interacted with something
# and the per-margin diagnostics (B_marg1/2, xi_gpd1/2) will say which margin.
#
# IMPORTANT: the tube's survFunc is a closure over inline transform helpers
# defined in the SIMULATION script's global environment; after readRDS in a
# fresh session those are not found. This script REDEFINES them and rebuilds
# survFunc explicitly from the stored dat + trimmed marginal fits.
library(dplyr)
library(matrixStats)
...
# NOTE: point save_full_path at wherever the 0.1.1.2 t4t10 bootmargs tubes were saved.
save_full_path <- '/pscratch/sd/j/jbbutler/isolines/simulations/extremal/0.1.1.2_bivt4_t4t10margins_gamma1_0.5_gamma2_0.67_bootmargs/'
...
# --- marginal / copula parameters (MUST match the 0.1.1.2 simulation) ---
df1 <- 4        # margin 1: t_4  (xi_1 = 1/4)
df2 <- 10       # margin 2: t_10 (xi_2 = 1/10)
...
std_blended_surv <- function(zpts, Z, qn) {
    ...
    tail_probs <- (rq / rp) * qn                 # xi = 1 on the standardized scale (AD)
...
print("Starting evaluation of raw (uncorrected) marginal-transform t4/t10 asymmetric-margin tubes...")
...
for (dist in dists) {
  for (n in ns) {
    ...
    true_isoline   <- data.frame(X1 = qt(copula_isoline[,1], df=df1),
...
if (length(results_list) > 0) {
    ...
    save_fname <- paste0(save_df_path, '0.1.1.0_bivt4_t4t10margins_gamma1_0.5_bootmargs.csv')
