...
# the AD margin-ladder simulations: LIGHT UNEQUAL GAUSSIAN TAILS: N(0,5) and N(0,10) margins, t_4 copula.
#
...
# NOTE: point save_full_path at wherever the 0.1.1.2 gauss5gauss10margs tubes were saved.
save_full_path <- '/pscratch/sd/j/jbbutler/isolines/simulations/extremal/0.1.1.2_bivt4_gauss5gauss10margs_gamma1_0.5_gamma2_0.67_bootmargs/'
...
# --- marginal parameters (LIGHT, UNEQUAL Gaussian tails) + copula ---
sd1 <- sqrt(5)                 # margin 1: N(0, 5)   (xi_1 = 0, Gumbel domain)
sd2 <- sqrt(10)                # margin 2: N(0, 10)  (xi_2 = 0, Gumbel domain)
copula_df <- 4                 # t-copula degrees of freedom (asymptotically DEPENDENT)
dists <- c('t4copula_gauss5gauss10margs')
lbs <- c(0, 0)
ubs <- c(200, 200)
...
for (dist in dists) {
  for (n in ns) {
    ...
    # TRUE p-isoline: same chain as the simulation script
        ...
        isoline <- data.frame(X1 = qnorm(copula_isoline[,1], mean = 0, sd = sd1),
                              X2 = qnorm(copula_isoline[,2], mean = 0, sd = sd2))
...
if (length(results_list) > 0) {
    ...
    save_fname <- paste0(save_df_path, '0.1.1.0_bivt4_gauss5gauss10margs_gamma1_0.5_bootmargs.csv')
