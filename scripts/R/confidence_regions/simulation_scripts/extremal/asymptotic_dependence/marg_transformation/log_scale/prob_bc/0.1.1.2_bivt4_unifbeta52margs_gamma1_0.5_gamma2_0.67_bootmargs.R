...
# BOUNDED, NON-DECAYING ENDPOINT: Uniform(0,1) and Beta(5,2), t_4-copula.
# The SALIENT margin is the Uniform: density = 1 at the upper endpoint 1, i.e.
# density does NOT decay to zero, giving true xi_1 = -1 -- OUTSIDE GPD-MLE
# regularity (xi > -1/2), the analytic sibling of the saturating relative-
# humidity margin. The GPD-MLE is expected to misbehave (xi -> -1 / endpoint
# collapse) EVEN under AD, with NO transition machinery to blame. Beta(5,2)
# (density ~ (1-x) at endpoint, xi_2 = -1/2 boundary) is the milder partner.
#
...
parser <- ArgumentParser(description = "Marginal-transform AD tube sims: non-decaying-endpoint Uniform/Beta(5,2) margins, t_4 copula, GPD margins, Pareto standardization.")
...
# --- marginal parameters (NON-DECAYING endpoint density) + copula ---
# margin 1: Uniform(0,1) -- density = 1 at endpoint 1, true xi_1 = -1 (OUTSIDE
#           GPD-MLE regularity; the saturating-margin analogue).
b2_shape1 <- 5; b2_shape2 <- 2    # margin 2: Beta(5,2), endpoint 1, xi_2 = -1/2 (boundary)
copula_df <- 4                    # t-copula degrees of freedom (asymptotically DEPENDENT)
dists <- c('t4copula_unifbeta52margs')
lbs <- c(0, 0)
ubs <- c(200, 200)
...
# sampler: t_4-copula with Uniform(0,1) and Beta(5,2) margins. For the Uniform
# margin the copula uniform IS the margin (qunif is the identity on (0,1)).
sample_t4copula_unifbeta <- function(n) {
    Sigma <- matrix(c(1, 0.7, 0.7, 1), 2, 2)
    Tm <- mvtnorm::rmvt(n, sigma = Sigma, df = copula_df)
    U  <- pt(Tm, df = copula_df)
    X1 <- U[,1]                                  # Uniform(0,1): qunif is identity
    X2 <- qbeta(U[,2], shape1 = b2_shape1, shape2 = b2_shape2)
...
for (i in 1:length(dists)) {
    dist <- dists[i]
    sampling_func <- sample_t4copula_unifbeta
    ...
    for (k in 1:length(ns)) {
        ...
        # TRUE p-isoline: t_4-copula survival isoline -> pt(copula_df) -> per-margin
        # quantiles. margin 1 = Uniform(0,1) (identity), margin 2 = Beta(5,2).
        ...
        isoline <- data.frame(X1 = copula_isoline[,1],
                              X2 = qbeta(copula_isoline[,2], shape1 = b2_shape1, shape2 = b2_shape2))
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '0.1.1.2_bivt4_unifbeta52margs_gamma1_0.5_gamma2_0.67_bootmargs.csv')
