...
# BOUNDED (DECAYING) MARGINS: Beta(2,3) and Beta(2,7), t_4-copula dependence.
# Both margins have FINITE endpoint 1 with density vanishing there -- true
# xi_j = -1/(beta_j - 1): Beta(2,3) -> xi = -1/2 (boundary of the asymptotic-
# normality regime), Beta(2,7) -> xi = -1/6 (comfortably interior, xi > -1/2).
#
...
parser <- ArgumentParser(description = "Marginal-transform AD tube sims: bounded Beta(2,3)/Beta(2,7) margins, t_4 copula, GPD margins, Pareto standardization.")
...
# sampler: t_4-copula with Beta(2,3) and Beta(2,7) margins. Copula from a bivariate
# t_4, mapped to uniforms via the t_4 CDF, then through the per-margin Beta quantiles.
sample_t4copula_beta <- function(n) {
    Sigma <- matrix(c(1, 0.7, 0.7, 1), 2, 2)
    Tm <- mvtnorm::rmvt(n, sigma = Sigma, df = copula_df)
    U  <- pt(Tm, df = copula_df)
    X1 <- qbeta(U[,1], shape1 = b1_shape1, shape2 = b1_shape2)
...
for (i in 1:length(dists)) {
    dist <- dists[i]
    sampling_func <- sample_t4copula_beta
    ...
    for (k in 1:length(ns)) {
        ...
        # Beta quantiles. Same t_4 copula as the data; margins Beta(2,3), Beta(2,7).
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '0.1.1.2_bivt4_beta23beta27margs_gamma1_0.5_gamma2_0.67_bootmargs.csv')
