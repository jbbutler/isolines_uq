...
# the AD margin-ladder simulations: BOUNDED (DECAYING) MARGINS: Beta(2,3) and Beta(2,7), t_4 copula.
#
# Loads each saved tube object from the 0.1.1.2 runs, reconstructs the survival
# estimator from the stored trimmed fits + data (the saved survFunc closure does
# not survive readRDS in a fresh session), swaps the RAW walls
# (raw_c_plus/minus_estimates) into the evaluation slots, and re-evaluates
# coverage against the same true isoline. All machinery is inlined VERBATIM
# from the simulation script (self-contained; full unconstrained GPD MLE, no
# fixes -- matching the original pipeline these tubes were built with).
#
# Diagnostics carried through per sim: xi_gpd1/2 (watch the regularity ladder:
# ~0 / (-1/2,-1/6) / (-1,-1/2)), fallback_frac, clamp_frac, B_marg1/2, b_marg.
#
...
# NOTE: point save_full_path at wherever the 0.1.1.2 beta23beta27margs tubes were saved.
save_full_path <- '/pscratch/sd/j/jbbutler/isolines/simulations/extremal/0.1.1.2_bivt4_beta23beta27margs_gamma1_0.5_gamma2_0.67_bootmargs/'
...
# --- marginal parameters (BOUNDED, decaying-density Beta margins) + copula ---
b1_shape1 <- 2; b1_shape2 <- 3    # margin 1: Beta(2,3), endpoint 1, xi_1 = -1/2 (boundary)
b2_shape1 <- 2; b2_shape2 <- 7    # margin 2: Beta(2,7), endpoint 1, xi_2 = -1/6 (interior)
copula_df <- 4                    # t-copula degrees of freedom (asymptotically DEPENDENT)
dists <- c('t4copula_beta23beta27margs')
lbs <- c(0, 0)
ubs <- c(200, 200)
...
for (dist in dists) {
  for (n in ns) {
    ...
    # TRUE p-isoline: same chain as the simulation script
        ...
        isoline <- data.frame(X1 = qbeta(copula_isoline[,1], shape1 = b1_shape1, shape2 = b1_shape2),
...
if (length(results_list) > 0) {
    ...
    save_fname <- paste0(save_df_path, '0.1.1.0_bivt4_beta23beta27margs_gamma1_0.5_bootmargs.csv')
