...
# COPULA LADDER, SLOW-DEPENDENCE CELL: t_50 copula with t_4/t_4 margins.
# The margins are the validated heavy-tailed configuration (fitted channel,
# known to self-insure); the COPULA second-order rate is dialed toward the
# rho -> 0 envelope: the t_nu dependence structure has second-order rate
# ~ -2/nu, so nu = 50 gives rho ~ -0.04 -- near the extremal (linear-in-log)
# bias profile the M0 pad is priced for. At accessible levels the copula is
# near-Gaussian (interior exponent ~ (1+0.7)/2 = 0.85) while the AD estimator
# projects with frozen exponent 1: a STRUCTURAL dependence-channel bias that
# no marginal refit can absorb. This is the stress test for the copula pad.
# MARGINAL-TRANSFORMATION PROCEDURE, asymptotic dependence (AD) version.
#
# Jimmy Butler, July 2026

# Pipeline (unchanged from the bivt4 template):
...
# sampler: t_50-copula with t_4 margins. Build the copula from a bivariate t_50,
# map to uniforms via the t_50 CDF, push through t_4 quantiles on both margins.
sample_t50copula_t4 <- function(n) {
    Sigma <- matrix(c(1, 0.7, 0.7, 1), 2, 2)
    Tm <- mvtnorm::rmvt(n, sigma = Sigma, df = copula_df)
    U  <- pt(Tm, df = copula_df)
    data.frame(X1 = qt(U[,1], df = marg_df), X2 = qt(U[,2], df = marg_df))
}
...
for (i in 1:length(dists)) {
    dist <- dists[i]
    sampling_func <- sample_t50copula_t4
    ...
    for (k in 1:length(ns)) {
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '0.1.1.2_bivt50copula_t4t4margs_gamma1_0.5_gamma2_0.67_bootmargs.csv')
