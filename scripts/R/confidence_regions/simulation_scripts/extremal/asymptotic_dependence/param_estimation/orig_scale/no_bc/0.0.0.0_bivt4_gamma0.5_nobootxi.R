...
parser <- ArgumentParser(description = "Args for Variance-Only (No Bias Correction) extremal simulations.")
...
computeExtremeRegion <- function(dat, alphas, p, B, gamma1=1/2, ncoords=50, lbs=c(0,0), progress_bar=FALSE, verbose=FALSE) {
    ...
    for (k in 1:B) {
        ...
        # NOTE: Using the original fixed xi_hat for projection, no re-estimation
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, 'exp0.1_ctrl.csv')
