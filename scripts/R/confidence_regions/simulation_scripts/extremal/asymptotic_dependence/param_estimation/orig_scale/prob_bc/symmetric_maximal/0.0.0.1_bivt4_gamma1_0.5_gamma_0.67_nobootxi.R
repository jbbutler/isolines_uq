...
parser <- ArgumentParser(description = "Args for Empirical Probability Gap bias bounding simulations.")
...
# using the Empirical Probability Gap to structurally expand the variance tube.
computeExtremeRegionBC_ProbabilityGap <- function(dat, alphas, p, B, gamma1=1/2, gamma2=2/3, ncoords=50, lbs=c(0,0), progress_bar=FALSE, verbose=FALSE) {
    ...
    if (verbose) print('Evaluating Empirical Gap for beta_max calibration.')
    ...
    # 3. Calculate Structural Bias components (incorporating correct negative inheritance)
    delta_P <- emp_probs_q2 - q2
    C_theta <- delta_P / (q2 * log(q1 / q2))
    beta_theta <- p * C_theta * log(q1 / p)
    
    # 4. Extract the Global Supremum
    beta_max <- max(abs(beta_theta))

    if (verbose) print('Estimating pure variance parameters via Centered Bootstrap.')
    ...
    # 5. Bootstrap Centering (Address non-linear finite-sample expectation bias)
    ...
    for (i in 1:length(alphas)) {
        ...
        # Save raw tracking values
        raw_c_plus_estimates[as.character(alpha)] <- c_plus_raw
        raw_c_minus_estimates[as.character(alpha)] <- c_minus_raw
        
        # Adjust quantiles structurally via beta_max
        c_plus_estimates[as.character(alpha)] <- c_plus_raw + beta_max
        c_minus_estimates[as.character(alpha)] <- c_minus_raw - beta_max
    }
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        parallelizedCode <- function(ind) {
            ...
            # use the new Empirical Gap bounding function
            regions <- computeExtremeRegionBC_ProbabilityGap(
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, 'extremal_coverage_probability_BC_gamma1_0.5_gamma2_0.67.csv')
