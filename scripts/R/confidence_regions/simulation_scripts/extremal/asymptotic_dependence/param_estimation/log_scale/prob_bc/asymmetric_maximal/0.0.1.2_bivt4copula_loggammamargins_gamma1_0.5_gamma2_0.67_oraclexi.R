...
# ORACLE-XI variant: xi is FIXED at the true value (1/4) everywhere -- no Hill
# estimation in the point estimate OR the bootstrap. Log-scale band + tube size.
#
# Purpose: test whether the log-gamma "good coverage" under ESTIMATED xi was Hill
# compensation. Fixing xi at the truth removes the compensating upward Hill bias,
# so if the good coverage was compensation, the UNCORRECTED tubes should collapse
# (and, at rho = 0, plausibly harder than the bivariate t).
#
...
parser <- ArgumentParser(description = "Oracle-xi Asymmetric Fractional Log-Prob sims: t4-copula, log-gamma margins.")
...
#   a != 1 gives a genuine slowly-varying log factor => rho = 0
lg_shape  <- 2      # a
lg_rate   <- 4      # b = 1/xi
copula_df <- 4      # t-copula degrees of freedom (nu)

# --- ORACLE xi (fixed everywhere; no Hill) ---
xi_true <- 1 / lg_rate      # = 0.25
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '0.0.1.2_bivt4copula_loggammamargins_gamma1_0.5_gamma2_0.67_oraclexi.csv')
