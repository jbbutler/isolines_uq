...
# ORACLE-XI variant: xi is FIXED at the true value (1/4 for bivariate t_4) everywhere --
# no Hill estimation in the point estimate OR the bootstrap. Log-scale band + tube size.
#
# Purpose: isolate whether fixed-xi failure is driven by structural bias or by the
# additive-scale blow-up. Log scale keeps the tube bounded (outer wall p*exp(c-) > 0),
# so this tests the bias in isolation from the boundedness pathology.
#
# DISTRIBUTION: bivariate t_4 (elliptical), xi = 1/4, rho = -1/2 (fast convergence).
#
...
parser <- ArgumentParser(description = "Oracle-xi Asymmetric Fractional Log-Prob sims: bivariate t_4.")
...
# --- ORACLE xi ---
# Bivariate t_nu (elliptical): radial tail index 1/xi = nu, so xi = 1/nu.
t_df   <- 4
xi_true <- 1 / t_df      # = 0.25; FIXED everywhere (no Hill)
...
# sampler for the bivariate t_4
sample_bivt4 <- function(n) {
    Sigma <- matrix(c(1, 0.7, 0.7, 1), 2, 2)
    X <- mvtnorm::rmvt(n, sigma = Sigma, df = t_df)
    data.frame(X1 = X[,1], X2 = X[,2])
}

computeExtremeRegionBC_OracleXi <- function(dat, alphas, p, B, gamma1=1/2, gamma2=2/3,
                                            ncoords=50, lbs=c(0,0), xi_fixed=0.25,
                                            progress_bar=FALSE, verbose=FALSE) {
    n_dat <- nrow(dat)
    q1 <- n_dat^(-gamma1); q2 <- n_dat^(-gamma2)

    # ORACLE: no Hill. xi is the fixed true value throughout.
    xi_hat <- xi_fixed
    ...
    for (k in 1:B) {
        ...
        # ORACLE: xi stays fixed in the bootstrap too (no re-estimation)
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        isoline <- drawIsoline(dist='bivt', numCoords=n_coords, gridUbs=ubs, gridLbs=lbs, prob=p)
        ...
        parallelizedCode <- function(ind) {
            dat <- sampling_func(n)
            regions <- computeExtremeRegionBC_OracleXi(
                dat=dat, alphas=alphas, p=p, B=B, gamma1=gamma1, gamma2=gamma2,
                ncoords=n_coords, lbs=lbs, xi_fixed=xi_true)
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, '0.0.1.2_bivt4_oraclexi_gamma1_0.5_gamma2_0.67.csv')
