...
# 
# This version implements the Fractional Log-Probability approach to ensure the lower probability 
# bounds are strictly positive, preventing the spatial tube from blowing up to infinity.
#
# Jimmy Butler
# June 2026
...
parser <- ArgumentParser(description = "Args for Fractional Log-Probability bias bounding simulations.")
...
# using the Fractional Log-Probability method.
computeExtremeRegionBC_FractionalLog <- function(dat, alphas, p, B, gamma1=1/2, gamma2=2/3, ncoords=50, lbs=c(0,0), progress_bar=FALSE, verbose=FALSE) {
    ...
    # 4. Extract the Global Fractional Supremum
    beta_frac_max <- max(abs(beta_frac_theta))
    ...
    for (i in 1:length(alphas)) {
        ...
        # Adjust quantiles structurally via beta_frac_max, then map to standard scale
        # Because exp(x) > 0, the total probability will never be <= 0.
        c_plus_estimates[as.character(alpha)] <- p * exp(c_plus_log + beta_frac_max) - p
        c_minus_estimates[as.character(alpha)] <- p * exp(c_minus_log - beta_frac_max) - p
    }
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        parallelizedCode <- function(ind) {
            ...
            # use the new Fractional Log bounding function
            regions <- computeExtremeRegionBC_FractionalLog(
            ...
            res_df <- data.frame(
                ...
                beta_frac_max = regions$beta_frac_max,
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, 'extremal_coverage_fractional_log_BC_gamma1_0.5_gamma2_0.67.csv')
