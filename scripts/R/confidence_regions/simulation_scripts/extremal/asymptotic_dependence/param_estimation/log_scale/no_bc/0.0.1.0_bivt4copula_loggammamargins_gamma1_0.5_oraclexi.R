# Script to loop through saved ORACLE-XI log-gamma (t_4-copula) confidence tubes
# and evaluate un-bias-corrected coverage.
#
# Loads the saved tube objects from the oracle-xi t4-copula / log-gamma-margin
# simulations (0.0.1.2 oraclexi), swaps the RAW (variance-only) walls into the
# evaluation slots, and re-evaluates coverage of the UNCORRECTED tubes against the
# true log-gamma isoline. Also records the size of the raw tubes.
#
# The tubes were built with xi FIXED at the truth (1/4), so any raw-vs-corrected
# coverage gap here reflects the second-order (rho = 0) bias alone, with the
# compensating upward Hill bias removed. Prediction: uncorrected coverage should
# collapse -- and, at rho = 0, plausibly harder than the bivariate t.
library(dplyr)
library(matrixStats)
...
# NOTE: point save_full_path at wherever the 0.0.1.2 oracle-xi log-gamma tubes were saved.
save_full_path <- '/pscratch/sd/j/jbbutler/isolines/simulations/extremal/0.0.1.2_bivt4copula_loggammamargins_gamma1_0.5_gamma2_0.67_oraclexi/'
...
print("Starting evaluation of raw (uncorrected) oracle-xi log-gamma tubes...")
...
for (dist in dists) {
  for (n in ns) {
    ...
    # (same construction as the 0.0.1.2 simulation script)
    ...
    for (i in 1:total_files) {
      ...
      #   r(s) = r_naive * (p / s)^{xi_hat}. xi_hat is the fixed oracle value (1/4).
      #   Log-scale => outer wall s_lo > 0 always.
      r_ref  <- tube_obj$r_naive     # estimated p-isoline radius, per angle
      xi_hat <- tube_obj$xi_hat      # fixed oracle EVI (scalar; elliptical-radial)
...
if (length(results_list) > 0) {
    ...
    save_fname <- paste0(save_df_path, '0.0.1.0_bivt4copula_loggammamargins_gamma1_0.5_oraclexi.csv')
