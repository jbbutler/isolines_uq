# Script to compute coverage for confidence tubes in the style of Mammen and Polonik (2013), where tube bounds
# are determined by variance in the extreme value estimator and a conservative assessment of bias using the theory
# of second order multivariate regular variation.
# 
# This version implements the Fractional Log-Probability approach with ASYMMETRIC global bounds
# to reduce conservatism. It splits the structural penalty into distinct upper and lower expansions.
#
# Jimmy Butler
# June 2026
...
parser <- ArgumentParser(description = "Args for Asymmetric Fractional Log-Probability bias bounding simulations.")
...
# using the Asymmetric Fractional Log-Probability method.
computeExtremeRegionBC_FractionalLogAsymmetric <- function(dat, alphas, p, B, gamma1=1/2, gamma2=2/3, ncoords=50, lbs=c(0,0), progress_bar=FALSE, verbose=FALSE) {
    ...
    if (verbose) print('Evaluating Empirical Gap for fractional bias calibration.')
    ...
    # 2. Evaluate Empirical Survival Function exactly on the q2 MRV isoline
    hit_q2 <- (outer(dat[,1], isoline_q2[,1], '>') & outer(dat[,2], isoline_q2[,2], '>')) * 1.0
    emp_probs_q2 <- colMeans(hit_q2)
    
    # 3. Calculate Fractional Structural Bias (Not multiplied by p!)
    delta_P <- emp_probs_q2 - q2
    C_theta <- delta_P / (q2 * log(q1 / q2))
    beta_frac_theta <- C_theta * log(q1 / p)
    
    # 4. Extract ASYMMETRIC Global Supremums
    # max(0, ...) ensures that if the bias doesn't swing in a particular direction,
    # we do not artificially shrink the tube bounds (penalty becomes exactly 0).
    # beta_frac_pos: Estimator under-predicts truth
    # beta_frac_neg: Estimator over-predicts truth
    ...
    # Matrix to store log probabilities
    ...
    for (k in 1:B) {
        ...
        # 1. RE-ESTIMATE XI FOR THIS BOOTSTRAP DRAW
        ...
        # 2. USE XI_BOOT TO EXTRAPOLATE
        tail_probs <- ((rq/rp)^(1/xi_boot))*qn
        final_probs <- ifelse(emp_probs >= qn, emp_probs, tail_probs)
        
        # TRANSFORMATION: Save the LOG of the bootstrap probabilities
    ...
    # Extract max/min deviations of the log-probabilities
    ...
    # 6. Bind Tube Bounds (Log Space -> Probability Space)
    for (i in 1:length(alphas)) {
        ...
        # Save raw tracking values in standard probability scale (Variance only)
        # Exponentiate the log-bound and shift by p for compatibility
        raw_c_plus_estimates[as.character(alpha)] <- p * exp(c_plus_log) - p
        raw_c_minus_estimates[as.character(alpha)] <- p * exp(c_minus_log) - p
        
        # Adjust quantiles structurally via ASYMMETRIC penalties, then map to standard scale
        # CROSS-MAPPED FIX: beta_frac_neg expands the upper bound, beta_frac_pos expands the lower bound.
...
for (i in 1:length(dists)) {
    ...
    for (k in 1:length(ns)) {
        ...
        parallelizedCode <- function(ind) {
            ...
            # use the new Asymmetric Fractional Log bounding function
        ...
        if (!is.null(args$save_df_path)) {
            ...
            save_fname <- paste0(args$save_df_path, 'extremal_coverage_fractional_log_asymmetric_BC_gamma1_0.5_gamma2_0.67_2.2.2.csv')
