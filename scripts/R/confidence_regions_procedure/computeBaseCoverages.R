...
ns <- c(1000, 5000, 10000, 25000, 50000)
...
for (i in 1:length(ps)) {
    isolines[[as.character(ps[i])]] <- isolineFunc(numCoords=numCoords, gridUbs=ubs, gridLbs=lbs, prob=ps[i]) 
}

# loading and saving paths
load_path <- paste0('/global/cscratch1/sd/jbbutler/sims/regions/base_confregs_', 
		    load_create_tag, '/', distribution, '/', grid_tag, '/')
save_path <- paste0('/global/cscratch1/sd/jbbutler/sims/regions/base_coverage_', 
                    load_create_tag, '/', distribution, '/', grid_tag, '/')
